...
export function ThemeHowItWorks() {
  const { howItWorks } = useSiteConfig();
  const header = useScrollReveal();
  const stepsReveal = useScrollReveal();

  return (
    <section id="how-it-works" className="py-20 sm:py-32 px-3 sm:px-4 max-w-7xl mx-auto theme-section rounded-[1.5rem] sm:rounded-[var(--radius-card-lg)] my-4 sm:my-10">
      <div ref={header.ref} className={`text-center mb-14 sm:mb-24 ${header.isVisible ? "sr-visible" : "sr-hidden"}`}>
        <div className="inline-block text-accent px-3 sm:px-4 py-1.5 sm:py-2 rounded-xl text-[10px] sm:text-xs font-bold uppercase tracking-widest mb-4 sm:mb-6" style={{ background: "var(--color-dark)", boxShadow: "var(--card-shadow)" }}>
          {howItWorks.badge}
        </div>
            ...
            <div
              className={`w-14 h-14 sm:w-20 sm:h-20 md:w-24 md:h-24 rounded-full flex items-center justify-center text-lg sm:text-xl md:text-2xl font-bold md:mb-8 group-hover:-translate-y-1 md:group-hover:-translate-y-2 transition-all duration-300 relative shrink-0 ${
                step.active
                  ...
                  : "theme-card group-hover:border-gray-200"
              }`}
            >
