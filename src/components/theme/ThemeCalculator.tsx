...
export function ThemeCalculator() {
  const { calculator } = useSiteConfig();
  const section = useScrollReveal({ threshold: 0.1 });
  const [income, setIncome] = useState(calculator.defaultValue);

  const rows = calculator.compute(income);
  ...
  return (
    <section id="calculator" className="py-16 sm:py-24 px-3 sm:px-4 max-w-7xl mx-auto my-4 sm:my-10">
      <div ref={section.ref} className={`bg-dark rounded-[1.5rem] sm:rounded-[var(--radius-card-lg)] p-6 sm:p-10 md:p-20 relative overflow-hidden text-white flex flex-col md:flex-row items-center gap-8 sm:gap-16 shadow-2xl sr-scale ${section.isVisible ? "sr-visible" : ""}`}>
        ...
        <div className="absolute bottom-0 left-0 w-[300px] sm:w-[500px] h-[300px] sm:h-[500px] bg-gradient-to-tr from-accent/10 via-transparent to-transparent rounded-full blur-3xl pointer-events-none transform -translate-x-1/3 translate-y-1/3" />
        ...
        <div className="w-full md:w-1/2 relative z-10">
          ...
          <ul className="space-y-3 sm:space-y-5 hidden sm:block">
            {calculator.benefits.map((b, i) => (
              <li key={i} className="flex items-center gap-3 sm:gap-4 text-gray-300 font-medium text-sm sm:text-base">
                <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-xs sm:text-sm font-bold" style={{ background: "var(--surface-on-dark)", border: "1px solid var(--surface-on-dark-border)" }}>{i + 1}</div>
                {b}
              </li>
            ))}
          </ul>
        ...
        {/* Calculator Widget */}
        <div className="w-full md:w-1/2 relative z-10">
          <div className="text-gray-900 rounded-2xl sm:rounded-[2rem] p-5 sm:p-8 md:p-12 shadow-widget relative" style={{ background: "var(--card-on-dark-bg)", border: "1px solid var(--card-on-dark-border)", backdropFilter: "var(--card-backdrop)", WebkitBackdropFilter: "var(--card-backdrop)" }}>
            ...
            <div className="text-3xl sm:text-5xl md:text-6xl font-medium tracking-tight mb-6 sm:mb-10 tabular-nums">
              {animIncome.toLocaleString(calculator.locale)}{" "}
              <span className="text-lg sm:text-2xl text-gray-400 font-medium ml-0.5 sm:ml-1">{calculator.currency}</span>
            </div>

            <div className="relative mb-8 sm:mb-12">
              <div
                className="absolute left-0 top-1/2 -translate-y-1/2 h-2 bg-accent rounded-l-lg pointer-events-none"
                style={{ width: `${((income - calculator.min) / (calculator.max - calculator.min)) * 100}%` }}
              />
              <input
                type="range"
                min={calculator.min}
            ...
            <div className="rounded-xl sm:rounded-2xl p-4 sm:p-6 mb-6 sm:mb-8 space-y-3 sm:space-y-4" style={{ background: "var(--card-bg)", border: "1px solid var(--card-border)" }}>
              {rows.map((row, i) => (
                <div key={i}>
                  {i > 0 && <div className="w-full h-px bg-gray-200 border-dashed mb-3 sm:mb-4" />}
                  <div className="flex justify-between items-center">
                    <span className="text-xs sm:text-sm font-semibold text-gray-500">{row.label}</span>
                    <AnimatedRow value={row.value} locale={calculator.locale} currency={calculator.currency} />
                  </div>
                </div>
              ))}
            </div>

            <div className="flex justify-between items-center pt-5 sm:pt-8" style={{ borderTop: "1px solid var(--card-border)" }}>
              <span className="font-bold text-gray-900 text-sm sm:text-lg">Total pro Jahr</span>
              <span className="text-xl sm:text-3xl font-bold tracking-tight text-white px-3 sm:px-5 py-2 sm:py-3 rounded-xl sm:rounded-2xl shadow-lg" style={{ background: "var(--color-dark)", border: "1px solid var(--surface-on-dark-border)" }}>
                <span className="text-accent tabular-nums">{animTotal.toLocaleString(calculator.locale)}</span> {calculator.currency}
              </span>
            </div>
...
function AnimatedRow({ value, locale, currency }: { value: number; locale: string; currency: string }) {
  const anim = useAnimatedNumber(value, 400);
  return (
    <span className="font-bold text-gray-900 text-base sm:text-lg tabular-nums">
      {anim.toLocaleString(locale)} {currency}
    </span>
  );
}
